...
func (c *photoController) GetByID(context *gin.Context) {
	...
	if err != nil {
		res := helpers.BuildErrorResponse("Parameter id tidak ditemukan", err.Error(), helpers.EmptyObj{})
	...
	if (photo == models.Photo{}) {
		res := helpers.BuildErrorResponse("Data tidak ditemukan", "Tidak ada data dengan id tsb", helpers.EmptyObj{})
...
func (c *photoController) Insert(context *gin.Context) {
	...
	if errData != nil {
		res := helpers.BuildErrorResponse("Maaf, gagal dalam memproses request", errData.Error(), helpers.EmptyObj{})
...
func (c *photoController) Update(context *gin.Context) {
	...
	if c.photoHelper.IsAllowedToEdit(userID, photoUpdateData.ID) {
		...
		response := helpers.BuildErrorResponse("Anda tidak memiliki izin", "Anda bukan pemilik", helpers.EmptyObj{})
...
func (c *photoController) Delete(context *gin.Context) {
	...
	id, err := strconv.ParseUint(context.Param("id"), 0, 0)
	if err != nil {
		response := helpers.BuildErrorResponse("Gagal mendapatkan id", "Tidak parameter id yg ditemukan", helpers.EmptyObj{})
